...
public class SmppSessionsImpl implements SmppSessions {
    ...
    public SmppSessionsImpl(SmppServerResourceAdaptor smppServerResourceAdaptor) {
        ...
        if (tracer == null) {
            tracer = this.smppServerResourceAdaptor.getRAContext()
                    .getTracer(SmppSessionHandlerInterfaceImpl.class.getSimpleName());
        }
    ...
    protected class SmppSessionHandlerInterfaceImpl implements SmppSessionHandlerInterface {
        ...
        public SmppSessionHandler createNewSmppSessionHandler(Esme esme) {
            ...
            RequestSender requestThread = new RequestSender(smppServerResourceAdaptor, tracer, esme,
                    "SMPP ESME Request Sender " + esme.getName(), SMPP_REQUEST_TIMEOUT);
            ResponseSender responseThread = new ResponseSender(smppServerResourceAdaptor, tracer,
                    "SMPP ESME Response Sender " + esme.getName(), SMPP_RESPONSE_TIMEOUT);
    ...
    protected class SmppSessionHandlerImpl implements SmppSessionHandler {
            ...
            try {
                ...
                switch (pduRequest.getCommandId()) {
                    ...
                    case SmppConstants.CMD_ID_SUBMIT_SM:
                        ...
                        if (!this.esme.isSourceAddressMatching(sourceAddress)) {
                            tracer.warning(String.format(
                                    "Incoming SUBMIT_SM's sequence_number=%d source_addr_ton=%d source_addr_npi=%d source_addr=%s doesn't match with configured ESME name=%s source_addr_ton=%d source_addr_npi=%d source_addr=%s",
                        ...
                        smppServerResourceAdaptor.fireEvent(EventsType.SUBMIT_SM, smppServerTransaction.getActivityHandle(),
                                submitSm);
                    ...
                    case SmppConstants.CMD_ID_DATA_SM:
                        ...
                        if (!this.esme.isSourceAddressMatching(sourceAddress)) {
                            tracer.warning(String.format(
                                    "Incoming DATA_SM's sequence_number=%d source_addr_ton=%d source_addr_npi=%d source_addr=%s doesn't match with configured ESME name=%s source_addr_ton=%d source_addr_npi=%d source_addr=%s",
                    ...
                    case SmppConstants.CMD_ID_DELIVER_SM:
                        ...
                        if (!this.esme.isSourceAddressMatching(sourceAddress)) {
                            tracer.warning(String.format(
                                    "Incoming DELIVER_SM's sequence_number=%d source_addr_ton=%d source_addr_npi=%d source_addr=%s doesn't match with configured ESME name=%s source_addr_ton=%d source_addr_npi=%d source_addr=%s",
                        ...
                        smppServerTransaction = new SmppTransactionImpl(pduRequest, this.esme, smppServerTransactionHandle,
                                smppServerResourceAdaptor);
                        smppServerResourceAdaptor.startNewSmppServerTransactionActivity(smppServerTransaction);
                        smppServerResourceAdaptor.fireEvent(EventsType.DELIVER_SM, smppServerTransaction.getActivityHandle(),
                                (DeliverSm) pduRequest);
                        return null;
                    ...
                    case SmppConstants.CMD_ID_SUBMIT_MULTI:
                        ...
                        if (!this.esme.isSourceAddressMatching(sourceAddress)) {
                            tracer.warning(String.format(
                                    "Incoming SUBMIT_MULTI's sequence_number=%d source_addr_ton=%d source_addr_npi=%d source_addr=%s doesn't match with configured ESME name=%s source_addr_ton=%d source_addr_npi=%d source_addr=%s",
                        ...
                        smppServerResourceAdaptor.startNewSmppServerTransactionActivity(smppServerTransaction);
                        smppServerResourceAdaptor.fireEvent(EventsType.SUBMIT_MULTI, smppServerTransaction.getActivityHandle(),
                                submitMulti);
        ...
        public void fireChannelUnexpectedlyClosed() {
            tracer.severe(String.format(
                    "Rx : fireChannelUnexpectedlyClosed for SmppSessionImpl=%s Default handling is to discard an unexpected channel closed",
                    this.esme.getName()));
        ...
        public void fireExpectedPduResponseReceived(PduAsyncResponse pduAsyncResponse) {
            ...
            if (smppServerTransaction == null) {
                tracer.severe(String.format(
                        "Rx : fireExpectedPduResponseReceived for SmppSessionImpl=%s PduAsyncResponse=%s but SmppTransactionImpl is null",
            ...
            try {
                switch (pduResponse.getCommandId()) {
                    ...
                    case SmppConstants.CMD_ID_DATA_SM_RESP:
                        smppServerResourceAdaptor.fireEvent(EventsType.DATA_SM_RESP, smppServerTransaction.getActivityHandle(),
                                (DataSmResp) pduResponse);
                        break;
                    ...
                    default:
                        tracer.severe(String.format(
                                "Rx : fireExpectedPduResponseReceived for SmppSessionImpl=%s PduAsyncResponse=%s but PduResponse is unidentified. Event will not be fired ",
                                this.esme.getName(), pduAsyncResponse));
                        break;
                }
        ...
        public void firePduRequestExpired(PduRequest pduRequest) {
            ...
            if (smppServerTransaction == null) {
                tracer.severe(String.format(
                        "Rx : firePduRequestExpired for SmppSessionImpl=%s PduRequest=%s but SmppTransactionImpl is null",
            ...
            try {
                smppServerResourceAdaptor.fireEvent(EventsType.REQUEST_TIMEOUT, smppServerTransaction.getActivityHandle(),
                        event);
            } catch (Exception e) {
                tracer.severe(String.format("Received firePduRequestExpired. Error while processing PduRequest=%s", pduRequest),
                        e);
            } finally {
                smppServerResourceAdaptor.endActivity(smppServerTransaction);
                smppServerTransaction.releaseSemaphore();
        ...
        public void fireRecoverablePduException(RecoverablePduException recoverablePduException) {
            ...
            if (smppServerTransaction == null) {
                tracer.severe(
                        String.format("Rx : fireRecoverablePduException for SmppSessionImpl=%s but SmppTransactionImpl is null",
                                this.esme.getName()),
            ...
            try {
                ...
                tracer.severe(
                        String.format("Received fireRecoverablePduException. Error while processing RecoverablePduException=%s",
                                recoverablePduException),
                        e);
            } finally {
                smppServerResourceAdaptor.endActivity(smppServerTransaction);
        ...
        public void fireUnknownThrowable(Throwable throwable) {
            ...
            if (defaultSession != null) {
                ...
                tracer.severe("Received fireUnknownThrowable with undifined defaultSession. Message: " + throwable.getMessage(),
                        throwable);
            }
